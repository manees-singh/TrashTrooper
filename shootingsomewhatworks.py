...
class Monster(pygame.sprite.Sprite):
    ...
    def decrease_monster_health(self):
        self.health -= 20
    def update(self):
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, pos,camera_offset_x, camera_offset_y):
        super().__init__()
        self.image = pygame.Surface((BULLET_SIZE, BULLET_SIZE))
        self.image.fill(BLACK)
        self.rect = self.image.get_rect(center=pos)  # Set bullet position to player's position
...
def play_game():
    ...
    bullets = pygame.sprite.Group()  # Define the bullets group

    fired = False  # Flag to track bullet firing cooldown
    ...
    while running:
        ...
        bullet_monster_collisions = pygame.sprite.groupcollide(bullets, all_sprites, True, False)

        for bullet, monsters in bullet_monster_collisions.items():
            for monster in monsters:
                if isinstance(monster, Monster):
            # Decrease the health of the monster
                    monster.decrease_monster_health()  # Adjust the amount as needed
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    bullet = Bullet(player.rect.midtop, camera_offset_x, camera_offset_y)
                    bullets.add(bullet)
        ...
        if isinstance(monster2, Monster):
        ...
        if isinstance(monster3, Monster):
